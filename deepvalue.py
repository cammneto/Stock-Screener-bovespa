...
if len(df1)==len(df2)==len(df3):
    ...
    print(f'Status Invest =  {len(df1)} | Investsite =  {len(df2)} | Fundamentus =  {len(df3)}')
    raise SystemExit('DF Size ERROR: Dataframes have differente sizes')

df=pd.concat([df1,df2,df3],axis=1, ignore_index=True)
df=rec_jud(df)
try:
    vol = float(input('Digite o limite mínimo de volume médio de transação diário em milhões de R$ (ex: para R$ 500mil digite 0.5 | Default Value: 0.2 )'))
except ValueError:
    vol = 0.2 
df=low_vol(df,vol)
df=neg_ebit(df)
...
df_std = pd.DataFrame({'ev1':df[3],'ev2':df[10],'ev3':df[17]}).std(axis=1)
...
df_new=pd.DataFrame({'Empresa':df[0], 'Ticker':df[1], 'Price':df[9], 'EV/EBIT':round(((df[3]+df[10]+df[17])/3)+df_std,2), 'M.EBIT':round((df[4]+df[11]+df[18])/3,2), 'Vol.(Mi)3m':df[12], 'Std':round(df_std,2),'E.Y.':round((df[3]+df[10]+df[17])/3,2)})
...
#### OUTPUT SCREEN ####
rank = pd.concat([df1.iloc[0:,[0,2,4]], df2.iloc[0:,[0,2,4]], df3.iloc[0:,[0,2,4]]], axis=1)
...
rank = df_new.iloc[0:,[0,2,5,6,1,4]]
rank.index = rank.index +1
print('|-------------------------------------------------------------|')
print('|                         Average                             |')
print('|-------------------------------------------------------------|')
